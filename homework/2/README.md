...
**creates a horizontal cell array for string "abdc" for variable e; combines all characters into 1 string**

$$ f = ['abcd']
f =
    'abcd'

**creates a horizontal cell array for string "abdc" for variable f**
...
**that special quotation mark type character is not recognized by matlab2017**
...
**creates a cell array (horizontally) that displays the variables values**
...
**If we type cast a larger integer than the one allowed by MATLAB, the program will only display up to the largest allowed integer, and vice versa for the smallest integer**
...
**left division, so normal division 1 by 2**
...
**adding corresponding values from matrix of the variables, for example, 1+ -1 = 0 is the first value in the new matrix**
...
**Element multiplication - Multiplying corresponding values from matrix of the variables, for example, 1 * -1 = -1 is the first value in the new matrix**
...
a1 = b (**the variable b is undefined, so you must change to number or define it **)  
...
disp tan(pi) (** to make it easier, make new variable names A than is tan(pi) and disp A later);
c = 4^3^2^3; (** put parenthesis around every power**)
_ (**must set a variable equal to something else**) = ((c-78564)/c + 32))(**remove extra parenthesis**);
year = 2017;
...
beginning = stuff(0) (** cannot be 0 because 0 isn’t the starting number in MATLAB**);
...
class = 'INF1100, gr 2";
continue_ = x > 0;
fox == false; (**fox doesn’t have a value, so the second equals is not necessary**)
wolf = fox == true;
...
**This happens because the script file myscript is not in the current working directory I am on as (as you can see under ls); therefore, MATLAB
doesn’t know what myscript is to run it**
